...
import { Award, BarChart3, BookOpen, CheckCircle2, Clock, Flame, LayoutDashboard, ListChecks, Moon, Plus, RotateCcw, Target, Trash2 } from "lucide-react";
...
const focusModes: Array<{ key: FocusMode; label: string; minutes: number }> = [
  ...
  { key: "shortBreak", label: "5 分钟休息", minutes: 5 },
    ...
    <div className="min-h-screen bg-mist text-ink">
      <aside className="fixed left-0 top-0 hidden h-full w-64 border-r border-gray-200 bg-white/85 p-5 backdrop-blur-xl lg:block">
        ...
        <nav className="mt-8 space-y-1">
          {nav.map((item) => (
            <a key={item.key} href={item.href} onClick={(event) => { event.preventDefault(); setTab(item.key); }} className={cn("flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition", tab === item.key ? "bg-klein text-white" : "text-gray-600 hover:bg-gray-100")}>
      ...
      <main className={cn("mx-auto max-w-7xl px-4 py-5 lg:ml-64 lg:px-8", !ready && "opacity-60")}>
        <div className="mb-5 rounded-xl border border-gray-200 bg-white/90 p-3 shadow-sm lg:hidden">
          ...
          <div className="flex gap-2 overflow-x-auto">{nav.map((item) => <button key={item.key} onClick={() => setTab(item.key)} className={cn("shrink-0 rounded-lg px-3 py-2 text-sm", tab === item.key ? "bg-klein text-white" : "bg-gray-100 text-gray-600")}>{item.label}</button>)}</div>
        </div>
...
function Dashboard({ state, overall, completedTasks, focusToday, reviewDone, setTab }: { state: AppState; overall: number; completedTasks: number; focusToday: FocusSession[]; reviewDone: boolean; setTab: (tab: Tab) => void }) {
  ...
  const categoryData = state.tasks.reduce<Array<{ name: Category; value: number }>>((list, task) => {
    const found = list.find((item) => item.name === task.category);
    if (found) found.value += 1;
    else list.push({ name: task.category, value: 1 });
    return list;
  }, []);
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl bg-klein p-7 text-white shadow-lift">
        <p className="text-sm text-white/70">{today()}</p>
        <h2 className="mt-3 text-4xl font-semibold tracking-tight">黄帝 Lv. {Math.max(...state.attributes.map((a) => a.level))}</h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-white/80">今日修炼正在推进。完成一个最小任务，也是在稳定变强。</p>
        <div className="mt-5 grid gap-4 md:grid-cols-3"><Metric title="今日成长值" value={`+${expToday} EXP`} dark /><Metric title="今日总进度" value={`${overall}%`} dark /><Metric title="复盘状态" value={reviewDone ? "已完成" : "待填写"} dark /></div>
        <Progress className="mt-6 bg-white/20 [&>div]:bg-white" value={overall} />
      </section>
      <div className="grid gap-4 md:grid-cols-5"><Metric title="任务完成率" value={`${percent(completedTasks, todayTasks.length)}%`} /><Metric title="英语掌握率" value={`${mastered}%`} /><Metric title="番茄钟" value={`${focusToday.length} 个`} /><Metric title="专注总时长" value={`${focusToday.reduce((s, x) => s + x.durationMinutes, 0)} 分`} /><Metric title="最近成就" value={recentAchievement?.title ?? "待解锁"} /></div>
      <AttributePanel attributes={state.attributes} />
      <div className="grid gap-5 xl:grid-cols-2">
        <ChartCard title="最近 7 天总进度"><AreaChart data={weekly}><XAxis dataKey="day" /><Tooltip /><Area type="monotone" dataKey="progress" stroke="#002FA7" fill="#002FA7" fillOpacity={0.12} /></AreaChart></ChartCard>
        <ChartCard title="任务分类占比"><PieChart><Pie data={categoryData} dataKey="value" nameKey="name" innerRadius={58} outerRadius={88}>{categoryData.map((_, index) => <Cell key={index} fill={chartColors[index % chartColors.length]} />)}</Pie><Tooltip /></PieChart></ChartCard>
      </div>
      <Card className="p-5"><div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-semibold">今日重点任务</h3><Button variant="secondary" onClick={() => setTab("review")}>写复盘</Button></div><div className="grid gap-3 md:grid-cols-3">{todayTasks.slice(0, 3).map((task) => <div key={task.id} className="rounded-xl bg-gray-50 p-4"><div className="flex justify-between gap-3"><b>{task.title}</b><span className="text-sm text-klein">{task.progress}%</span></div><p className="mt-1 text-xs text-gray-500">{task.category} · {task.priority}</p><Progress className="mt-3" value={task.progress} /></div>)}</div></Card>
    </div>
  );
}
...
function TaskCard({ task, updateTask, remove }: { task: Task; updateTask: (id: string, patch: Partial<Task>) => void; remove: (id: string) => void }) {
  ...
  return <div className={cn("rounded-xl border border-gray-100 bg-gray-50 p-4 transition hover:shadow-sm", task.status === "已完成" && "opacity-60")}><button className="w-full text-left" onClick={() => setOpen(!open)}><div className="flex justify-between gap-3"><b>{task.title}</b><span className="text-sm text-klein">{task.progress}%</span></div><p className="mt-1 text-xs text-gray-500">{task.category} · {task.priority} · 预计 {task.estimatedMinutes} 分</p></button><Progress className="mt-3" value={task.progress} />{open ? <div className="mt-3 grid gap-2"><Input value={task.title} onChange={(e) => updateTask(task.id, { title: e.target.value })} /><Textarea value={task.note} onChange={(e) => updateTask(task.id, { note: e.target.value })} placeholder="备注" /><input className="accent-klein" type="range" min={0} max={100} step={5} value={task.progress} onChange={(e) => updateTask(task.id, { progress: Number(e.target.value) })} /><div className="grid grid-cols-2 gap-2">{statusList.map((next) => <Button key={next} variant={next === task.status ? "primary" : "secondary"} onClick={() => updateTask(task.id, { status: next, progress: next === "已完成" ? 100 : task.progress })}>{next}</Button>)}</div><Button variant="danger" onClick={() => remove(task.id)}><Trash2 size={16} />删除</Button></div> : null}</div>;
}
...
function English({ state, patchState, notify }: { state: AppState; patchState: (patch: Partial<AppState>) => void; notify: (message: string) => void }) {
  ...
  function addBulk() {
    ...
    const now = new Date().toISOString();
    const next: Word[] = rows.map((row) => { const [word, ...rest] = row.split(/\s+/); return { id: uid("word"), word, meaning: rest.join(" ") || "待补充释义", date: today(), familiarity: 0, wrongCount: 0, correctCount: 0, createdAt: now, updatedAt: now }; });
    patchState({ words: [...next, ...state.words] });
    setBulk("");
    notify(`已添加 ${next.length} 个单词`);
  }
  ...
  function check() {
    ...
    const words = state.words.map((word) => word.id === current.id ? { ...word, familiarity: clamp(word.familiarity + (ok ? 1 : -1), 0, 5), correctCount: word.correctCount + (ok ? 1 : 0), wrongCount: word.wrongCount + (ok ? 0 : 1), updatedAt: new Date().toISOString() } : word);
    patchState({ words, attributes: ok ? addExp(state.attributes, "english", 2) : state.attributes });
    setAnswer("");
    notify(ok ? "拼写正确，英语力 +2" : `错误，正确答案是 ${current.word}`);
  }
...
function WordGrid({ words, updateWord, removeWord }: { words: Word[]; updateWord: (id: string, patch: Partial<Word>) => void; removeWord: (id: string) => void }) {
  return <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{words.map((word) => <div key={word.id} className="rounded-xl bg-gray-50 p-4"><Input value={word.word} onChange={(e) => updateWord(word.id, { word: e.target.value })} /><Input className="mt-2" value={word.meaning} onChange={(e) => updateWord(word.id, { meaning: e.target.value })} /><div className="mt-3 flex items-center gap-3"><input className="w-full accent-klein" type="range" min={0} max={5} value={word.familiarity} onChange={(e) => updateWord(word.id, { familiarity: Number(e.target.value) })} /><span className="text-xs text-klein">{word.familiarity}/5</span></div><Button className="mt-3" variant="danger" onClick={() => removeWord(word.id)}>删除</Button></div>)}</div>;
}
...
function Focus({ state, patchState, notify }: { state: AppState; patchState: (patch: Partial<AppState>) => void; notify: (message: string) => void }) {
  ...
  function finish(completed: boolean) {
    ...
    const session: FocusSession = { id: uid("focus"), taskId: task?.id, taskTitle: task?.title, category: task?.category, mode, durationMinutes: minutes, completed, distractionNote: distraction, reflection, date: today(), startedAt: new Date(Date.now() - minutes * 60000).toISOString(), endedAt: new Date().toISOString() };
    const attributes = completed && mode === "focus" ? addExp(state.attributes, categoryToAttribute(task?.category), Math.max(5, Math.round(minutes / 10) * 5)) : state.attributes;
    patchState({ focusSessions: [session, ...state.focusSessions], attributes, tasks: task ? state.tasks.map((item) => item.id === task.id ? { ...item, actualMinutes: item.actualMinutes + minutes, updatedAt: new Date().toISOString() } : item) : state.tasks });
    setSeconds(minutes * 60);
    notify(completed ? "本轮专注已记录" : "已保存本轮记录");
  }
...
function Stats({ state }: { state: AppState }) {
  ...
  return <div><Title icon={BarChart3} title="成长统计" sub="任务、英语、专注和属性趋势" /><div className="mb-5 grid gap-4 md:grid-cols-4"><Metric title="完成数" value={`${completed}`} /><Metric title="平均完成率" value={`${avg}%`} /><Metric title="英语单词" value={`${state.words.length}`} /><Metric title="拼写正确率" value={`${percent(spellingCorrect, spellingTotal)}%`} /><Metric title="错词数量" value={`${state.words.filter((w) => w.wrongCount > 0).length}`} /><Metric title="专注总时长" value={`${state.focusSessions.reduce((s, x) => s + x.durationMinutes, 0)} 分`} /><Metric title="记录天数" value={`${new Set(state.reviews.map((r) => r.date)).size} 天`} /><Metric title="阅读完成" value={`${state.readings.filter((r) => r.completed).length} 篇`} /></div><div className="grid gap-5 xl:grid-cols-2"><ChartCard title="最近 7 天进度趋势"><AreaChart data={weekly}><XAxis dataKey="day" /><Tooltip /><Area dataKey="progress" stroke="#002FA7" fill="#002FA7" fillOpacity={0.12} /></AreaChart></ChartCard><ChartCard title="分类任务占比"><PieChart><Pie data={categoryData} dataKey="value" nameKey="name" innerRadius={60} outerRadius={90}>{categoryData.map((_, i) => <Cell key={i} fill={chartColors[i % chartColors.length]} />)}</Pie><Tooltip /></PieChart></ChartCard><ChartCard title="属性成长"><BarChart data={state.attributes.map((a) => ({ name: a.name, exp: a.level * 100 + a.exp }))}><XAxis dataKey="name" /><YAxis /><Tooltip /><Bar dataKey="exp" fill="#002FA7" radius={[8, 8, 0, 0]} /></BarChart></ChartCard><ChartCard title="本周专注"><BarChart data={weekly}><XAxis dataKey="day" /><YAxis /><Tooltip /><Bar dataKey="focus" fill="#16A34A" radius={[8, 8, 0, 0]} /></BarChart></ChartCard></div></div>;
}

function Achievements({ achievements }: { achievements: Achievement[] }) {
  return <div><Title icon={Award} title="成就图鉴" sub="显示已解锁、未解锁、解锁时间和达成条件" /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{achievements.map((achievement) => <Card key={achievement.id} className={cn("p-5 transition hover:-translate-y-0.5 hover:shadow-sm", achievement.unlocked && "border-klein/30 bg-klein/5")}><div className="flex items-start gap-3"><div className={cn("rounded-xl p-2", achievement.unlocked ? "bg-klein text-white" : "bg-gray-100 text-gray-400")}><Award size={20} /></div><div><h3 className="font-semibold">{achievement.title}</h3><p className="mt-2 text-sm leading-6 text-gray-500">{achievement.description}</p><p className="mt-3 text-xs text-gray-500">{achievement.unlocked ? `已解锁：${achievement.unlockedAt?.slice(0, 10)}` : "未解锁"}</p></div></div></Card>)}</div></div>;
}

function Tabs<T extends string>({ current, setCurrent, items }: { current: T; setCurrent: (value: T) => void; items: Array<[T, string]> }) {
  return <div className="mb-5 flex flex-wrap gap-2">{items.map(([key, label]) => <Button key={key} variant={current === key ? "primary" : "secondary"} onClick={() => setCurrent(key)}>{label}</Button>)}</div>;
}
...
function ChartCard({ title, children }: { title: string; children: React.ReactElement }) { return <Card className="p-5"><h3 className="mb-4 text-lg font-semibold">{title}</h3><div className="h-72"><ResponsiveContainer width="100%" height="100%">{children}</ResponsiveContainer></div></Card>; }
...
function refreshAchievements(state: AppState): AppState {
  const todayFocus = state.focusSessions.filter((s) => s.date === today() && s.completed && s.mode === "focus").length;
  const completedCreative = state.tasks.filter((t) => t.status === "已完成" && t.category === "作品集").length;
  const readingsDone = state.readings.filter((r) => r.completed).length;
  const spellingCount = state.words.reduce((sum, w) => sum + w.correctCount + w.wrongCount, 0);
  const unlock = (achievement: Achievement) => {
    const conditions: Record<string, boolean> = { streak: new Set([...state.reviews.map((r) => r.date), ...state.focusSessions.map((s) => s.date)]).size >= 3, review_streak: state.reviews.length >= 7, word_count: state.words.length >= 100, spelling_count: spellingCount >= 50, reading_count: readingsDone >= 5, focus_day: todayFocus >= 4, focus_total: state.focusSessions.filter((s) => s.completed && s.mode === "focus").length >= 30, paper_streak: state.tasks.filter((t) => t.category === "毕业论文" && t.progress > 0).length >= 5, creative_count: completedCreative >= 10, progress_streak: state.tasks.length > 0 && state.tasks.reduce((sum, t) => sum + t.progress, 0) / state.tasks.length >= 80 };
    const shouldUnlock = conditions[achievement.conditionType] ?? achievement.unlocked;
    return shouldUnlock && !achievement.unlocked ? { ...achievement, unlocked: true, unlockedAt: new Date().toISOString() } : { ...achievement, unlocked: achievement.unlocked || shouldUnlock };
  };
  return { ...state, achievements: (state.achievements.length ? state.achievements : achievementSeed).map(unlock) };
}
