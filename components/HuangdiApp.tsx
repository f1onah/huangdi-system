...
export function HuangdiApp({ initialTab = "dashboard" }: { initialTab?: Tab }) {
  ...
  return (
    <div className="min-h-screen bg-mist text-ink">
      ...
      <main className={cn("mx-auto max-w-[1440px] px-3 pb-28 pt-4 sm:px-4 sm:py-6 lg:ml-72 lg:px-8", !ready && "opacity-60")}>
        <div className="sticky top-3 z-40 mb-5 rounded-[22px] border border-white/[0.08] bg-[#0B0F1A]/82 p-3 shadow-lift backdrop-blur-2xl lg:hidden">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-klein">Huangdi</p>
          <h1 className="mt-1 text-lg font-semibold text-white">黄帝养成系统</h1>
          <p className="mt-1 line-clamp-2 text-xs leading-5 text-white/55">陛下今日辛苦了！为大黄王朝之崛起而奋斗！</p>
        </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-white/[0.08] bg-[#0B0F1A]/92 px-2 pb-[max(env(safe-area-inset-bottom),0.55rem)] pt-2 shadow-[0_-20px_60px_rgba(0,0,0,0.38)] backdrop-blur-2xl lg:hidden">
        <div className="mx-auto grid max-w-md grid-cols-4 gap-1">
          {nav.map((item) => (
            <button key={item.key} onClick={() => setTab(item.key)} className={cn("flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-2xl px-1 text-[11px] leading-none transition", tab === item.key ? "bg-klein text-white shadow-[0_0_24px_rgba(0,47,167,0.42)]" : "text-white/55 hover:bg-white/[0.06] hover:text-white")}>
              <item.icon size={18} />
...
function EmperorAvatar() {
  return (
    <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-full border border-white/25 bg-white/[0.08] shadow-[0_0_34px_rgba(0,47,167,0.48)] ring-4 ring-white/10 sm:h-32 sm:w-32">
      <Image className="object-cover object-[center_26%]" src="/images/avatar.jpg" alt="黄帝头像" fill sizes="(min-width: 640px) 128px, 96px" priority />
    </div>
  );
}
...
function Dashboard({ state, overall, completedTasks, focusToday, reviewDone, setTab }: { state: AppState; overall: number; completedTasks: number; focusToday: FocusSession[]; reviewDone: boolean; setTab: (tab: Tab) => void }) {
  ...
  return <div className="animate-[fadeIn_0.5s_ease] space-y-5 sm:space-y-6"><Title icon={LayoutDashboard} title="养心殿" sub="总览今日修炼、事务进度与成长反馈" /><section className="overflow-hidden rounded-[24px] border border-white/[0.08] bg-[linear-gradient(135deg,rgba(0,47,167,0.95),rgba(11,15,26,0.92)_58%,rgba(0,200,150,0.14))] p-5 text-white shadow-glow backdrop-blur-2xl sm:rounded-[28px] sm:p-8"><div className="grid gap-5 lg:grid-cols-[1.4fr_0.6fr] lg:gap-8"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><EmperorAvatar /><div className="min-w-0 flex-1"><p className="text-xs leading-5 text-white/60 sm:text-sm">{today()} · 当前状态：稳步进阶中</p><h2 className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl">黄帝 Lv. {emperor.level}</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-white/72">今日修炼进度：{overall}% · 尚有 {pending} 项事务待处理。{mood}</p><Progress className="mt-6 bg-white/15 [&>div]:!bg-white sm:mt-8" value={overall} /></div></div><div className="rounded-glass border border-white/[0.1] bg-white/[0.08] p-4 backdrop-blur-2xl sm:p-5"><p className="text-sm text-white/55">本级经验</p><p className="mt-3 text-4xl font-semibold sm:text-5xl">{emperor.exp}<span className="text-lg text-white/45">/{emperor.nextLevelExp}</span></p><p className="mt-2 text-sm text-white/55">每过一天 +1 EXP</p><Progress className="mt-4 bg-white/15 [&>div]:!bg-white" value={(emperor.exp / emperor.nextLevelExp) * 100} /></div></div></section><DashboardWeather /><div className="grid grid-cols-2 gap-3 md:grid-cols-2 xl:grid-cols-5"><Metric title="事务完成率" value={`${percent(completedTasks, todayTasks.length)}%`} /><Metric title="词库掌握率" value={`${mastered}%`} onClick={() => setTab("english")} /><Metric title="番茄轮次" value={`${focusToday.length}`} onClick={() => setTab("focus")} /><Metric title="专注时长" value={`${focusToday.reduce((s, x) => s + x.durationMinutes, 0)} 分`} onClick={() => setTab("focus")} /><Metric title="最近功勋" value={recentAchievement?.title ?? "未达成"} onClick={() => setTab("achievements")} /></div><AttributePanel attributes={state.attributes} /><div className="grid gap-5 xl:grid-cols-2"><ChartCard title="近期修炼曲线"><AreaChart data={weekly}><defs><linearGradient id="progressFill" x1="0" x2="0" y1="0" y2="1"><stop offset="5%" stopColor="#002FA7" stopOpacity={0.55} /><stop offset="95%" stopColor="#002FA7" stopOpacity={0.03} /></linearGradient></defs><XAxis dataKey="day" /><Tooltip /><Area type="monotone" dataKey="progress" stroke="#7AA2FF" strokeWidth={3} fill="url(#progressFill)" /></AreaChart></ChartCard><ChartCard title="事务分布"><PieChart><Pie data={categoryData} dataKey="value" nameKey="name" innerRadius={44} outerRadius={78}>{categoryData.map((_, index) => <Cell key={index} fill={chartColors[index % chartColors.length]} />)}</Pie><Tooltip /></PieChart></ChartCard></div><Card className="p-5 sm:p-6"><div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h3 className="text-lg font-semibold text-white">今日重点事务</h3><Button className="w-full sm:w-auto" variant="secondary" onClick={() => setTab("review")}>前往省身</Button></div><div className="grid gap-4 md:grid-cols-3">{todayTasks.length ? todayTasks.slice(0, 3).map((task) => <MiniTask key={task.id} task={task} />) : <Empty text="暂无事务。可新拟一项今日修炼。" />}</div></Card></div>;
}
...
function Kitchen({ meals, setState, notify }: { meals: MealRecord[]; setState: Dispatch<SetStateAction<AppState>>; notify: (message: string) => void }) {
  ...
  return <div className="space-y-5 sm:space-y-6"><Title icon={Utensils} title="御膳房" sub="记录今日吃了什么、喝了什么，照顾好陛下的生活力" /><Card className="p-4 sm:p-6"><div className="grid gap-3 lg:grid-cols-[1fr_1fr_1fr_auto]"><Input value={draft.food} onChange={(event) => setDraft({ ...draft, food: event.target.value })} placeholder="今日吃了什么" /><Input value={draft.drink} onChange={(event) => setDraft({ ...draft, drink: event.target.value })} placeholder="今日喝了什么" /><Input value={draft.note} onChange={(event) => setDraft({ ...draft, note: event.target.value })} placeholder="备注" /><Button className="w-full" onClick={saveMeal}><Plus size={16} />记录御膳</Button></div></Card><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{todayMeals.length ? todayMeals.map((meal) => <Card key={meal.id} className="p-4 sm:p-5"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="text-sm text-white/45">{meal.date}</p><h3 className="mt-2 break-words text-xl font-semibold text-white">{meal.food || "未记食物"}</h3><p className="mt-2 break-words text-sm text-white/65">饮品：{meal.drink || "未记录"}</p>{meal.note ? <p className="mt-3 break-words text-sm text-white/55">备注：{meal.note}</p> : null}</div><button className="shrink-0 rounded-xl bg-white/[0.06] p-2 text-white/45 hover:text-white" onClick={() => removeMeal(meal.id)}><Trash2 size={16} /></button></div></Card>) : <Empty text="今日尚未记录御膳。" />}</div></div>;
}
...
function Focus({ state, patchState, notify }: { state: AppState; patchState: (patch: Partial<AppState>) => void; notify: (message: string) => void }) {
  ...
  return <div className="space-y-5 sm:space-y-6"><Title icon={Clock} title="静修室" sub="入静、出定、记录专注，将执行力沉淀为经验" /><div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]"><Card className="p-4 text-center sm:p-7"><div className="mb-5 grid grid-cols-1 gap-2 min-[380px]:grid-cols-3 sm:flex sm:flex-wrap sm:justify-center">{focusModes.map((item) => <Button key={item.key} className="w-full sm:w-auto" variant={mode === item.key ? "primary" : "secondary"} onClick={() => setMode(item.key)}>{item.label}</Button>)}</div><p className="mb-4 text-sm text-white/55">当前状态：{running ? "专注中" : "待入静"} · 剩余时间：{String(Math.floor(seconds / 60)).padStart(2, "0")}:{String(seconds % 60).padStart(2, "0")}</p><div className="mx-auto flex h-56 w-56 items-center justify-center rounded-full shadow-[0_0_80px_rgba(0,47,167,0.28)] sm:h-72 sm:w-72" style={{ background: `conic-gradient(#002FA7 ${progress * 3.6}deg,rgba(255,255,255,0.08) 0deg)` }}><div className="flex h-48 w-48 items-center justify-center rounded-full border border-white/[0.08] bg-[#0B0F1A] text-4xl font-semibold text-white sm:h-60 sm:w-60 sm:text-6xl">{String(Math.floor(seconds / 60)).padStart(2, "0")}:{String(seconds % 60).padStart(2, "0")}</div></div><Select className="mt-6" value={taskId} onChange={(e) => setTaskId(e.target.value)}><option value="">不绑定事务</option>{state.tasks.filter((task) => task.date === today()).map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}</Select><div className="mt-6 grid gap-3 sm:flex sm:flex-wrap sm:justify-center"><Button onClick={() => setRunning(!running)}>{running ? "暂停" : "入静"}</Button><Button variant="secondary" onClick={() => setSeconds(minutes * 60)}>重新开始</Button><Button variant="secondary" onClick={() => finish(true)}>出定</Button></div></Card><Card className="p-4 sm:p-7"><h3 className="text-lg font-semibold text-white">出定记录</h3><Textarea className="mt-4" value={reflection} onChange={(e) => setReflection(e.target.value)} placeholder="本轮完成了什么？下一轮准备做什么？" /><Textarea className="mt-4" value={distraction} onChange={(e) => setDistraction(e.target.value)} placeholder="是否分心？原因是什么？" /><div className="mt-6 grid grid-cols-1 gap-3 min-[380px]:grid-cols-3 md:grid-cols-3"><Metric title="今日入静" value={`${todaySessions.filter((s) => s.mode === "focus").length} 轮`} /><Metric title="专注时长" value={`${todaySessions.reduce((s, x) => s + x.durationMinutes, 0)} 分`} /><Metric title="累计修炼" value={`${state.focusSessions.filter((s) => s.completed).length} 轮`} /></div></Card></div></div>;
}
...
function ChartCard({ title, children }: { title: string; children: ReactElement }) { return <Card className="p-4 sm:p-6"><h3 className="mb-4 text-base font-semibold text-white sm:mb-5 sm:text-lg">{title}</h3><div className="h-56 sm:h-72"><ResponsiveContainer width="100%" height="100%">{children}</ResponsiveContainer></div></Card>; }
function Metric({ title, value, dark = false, onClick }: { title: string; value: string; dark?: boolean; onClick?: () => void }) {
  const content = <><p className={cn("text-xs sm:text-sm", dark ? "text-white/65" : "text-white/55")}>{title}</p><p className="mt-2 break-words text-2xl font-semibold tracking-tight text-white sm:mt-3 sm:text-3xl">{value}</p></>;
  return onClick ? <button onClick={onClick} className="rounded-glass border border-white/[0.08] bg-white/[0.05] p-4 text-left shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-2xl transition-all duration-300 hover:-translate-y-1 hover:border-klein/45 hover:shadow-glow sm:p-5">{content}</button> : <Card className={cn("p-4 sm:p-5", dark && "border-white/[0.12] bg-white/[0.08] text-white")}>{content}</Card>;
}
function Title({ icon: Icon, title, sub }: { icon: LucideIcon; title: string; sub?: string }) { return <div className="flex flex-col gap-2"><div className="flex items-start gap-3"><span className="shrink-0 rounded-2xl border border-klein/35 bg-klein/20 p-3 text-white shadow-[0_0_24px_rgba(0,47,167,0.28)]"><Icon size={21} /></span><div className="min-w-0"><h2 className="text-2xl font-semibold tracking-tight text-white sm:text-3xl">{title}</h2>{sub ? <p className="mt-1 text-sm leading-6 text-white/55">{sub}</p> : null}</div></div></div>; }
